  ...
  struct CheckedGetter {
    ...
    std::decay_t<T>&& operator()() {
      ...
      AT_ERROR(
          ...
          c10::demangle(arguments_.type_info().name()));
    }
...
class AnySpace {
 ...
 private:
  ...
  std::unique_ptr<AnySpacePlaceholder> make_holder(
      std::shared_ptr<SpaceType>&& space,
      ReturnType (Class::*)(ArgumentType...));
...
template <typename SpaceType, typename T>
AnySpace::AnySpace(SpaceType&& space) : AnySpace(std::make_shared<SpaceType>(std::forward<SpaceType>(space))) {}

template <typename SpaceType>
AnySpace::AnySpace(const SpaceHolder<SpaceType>& space_holder) : AnySpace(space_holder.ptr()) {}
...
bool AnySpace::contains(ArgumentType&& argument) {
  LAB_CHECK(!is_empty(), "Space holder is empty");
  return content_->contains(torch::nn::AnyValue(std::move(argument)));
}
...
template <typename SpaceType, typename Class, typename ReturnType, typename... ArgumentType>
std::unique_ptr<AnySpacePlaceholder> AnySpace::make_holder(
    std::shared_ptr<SpaceType>&& space,
    ReturnType (Class::* /*unused*/)(ArgumentType...)) {
  static_assert(
      torch::detail::check_not_lvalue_references<ArgumentType...>(),
...
SpaceType& AnySpace::get_() const {
  ...
  static_assert(
      utils::has_sample_and_contains<S>::value,
      "Can only call AnySpace::get<T> with a type T that has a "
      "sample and contains method");
  return get_(&S::sample);
}

template <typename SpaceType, typename ReturnType, typename... ArgumentType>
SpaceType& AnySpace::get_(ReturnType (SpaceType::* /*unused*/)(ArgumentType...)) const {
  if (typeid(SpaceType).hash_code() == type_info().hash_code()) {
    return *static_cast<AnySpaceHolder<SpaceType, ArgumentType...>&>(*content_).space;
  }
  AT_ERROR(
      "Attempted to cast space of type ",
      c10::demangle(type_info().name()),
      " to type ",
      c10::demangle(typeid(SpaceType).name()));
}
...
NamedAnySpace::NamedAnySpace(std::string name, M&& space)
    : NamedAnySpace(
          std::move(name),
          std::make_shared<typename std::remove_reference<M>::type>(std::forward<M>(space))) {}
