 ...
 private:
  template <
      ...
      typename = std::enable_if_t<
          !std::is_same_v<First, std::string> && !std::is_same_v<std::decay_t<First>, std::decay_t<const char (&)[]>>>>
...
std::vector<bool> SequentialImpl::contains(InputTypes&&... inputs) {
  LAB_CHECK(!is_empty(), "Container is empty");
  LAB_CHECK(sizeof...(InputTypes) == spaces_.size(), "Invalid size");
...
T& SequentialImpl::at(size_t index) {
  static_assert(utils::is_space<T>::value, "Can only call Sequential::at with an spaces::Space type");
  LAB_CHECK(index < size(), "Out of bound");
...
std::shared_ptr<T> SequentialImpl::ptr(size_t index) const {
  static_assert(utils::is_space<T>::value, "Can only call Sequential::ptr with an spaces::Space type");
  LAB_CHECK(index < size(), "Out of bound");
