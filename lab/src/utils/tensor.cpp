...
bool tensor_eq(const torch::Tensor& tensor1, const torch::Tensor& tensor2) {
  ...
  if (!tensor1.sizes().equals(tensor2.sizes()))
    return false;
...
torch::Tensor center_mean(const std::vector<double>& vec) {
  ...
  std::transform(vec.begin(), vec.end(), centered_vec.begin(), [mean](double val) { return val - mean; });
...
torch::Tensor standardize(const torch::Tensor& tensor) {
  LAB_CHECK(tensor.numel() > 1, "Expected tensor numel greater than 1, got " << tensor.numel());
...
torch::Tensor to_one_hot(const torch::Tensor& tensor, int64_t num_classes) {
  LAB_CHECK(tensor.dim() == 1, "Expected tensor dim is 1, got " << tensor.dim());
  LAB_CHECK(tensor.dtype() == torch::kInt64, "Only support tensor type torch::Int64_t");
  auto one_hot_tensor = torch::zeros(tensor.sizes().vec(), torch::kInt64)
                            .unsqueeze(-1)
                            .expand({-1, -1, num_classes})
