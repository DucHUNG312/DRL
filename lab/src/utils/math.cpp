...
std::vector<double> calc_returns(const std::vector<double>& rewards, const std::vector<bool>& dones, double gamma) {
  LAB_CHECK(
      rewards.size() == dones.size(),
      "Expected rewards and terminated memory to be equal in size, got " << rewards.size() << " and " << dones.size());
  std::vector<double> rets(rewards.size(), 0);
  double future_ret = 0;
  std::vector<bool> not_dones(dones.size());
  std::transform(dones.begin(), dones.end(), not_dones.begin(), [](bool done) { return !done; });
...
std::vector<double> calc_nstep_returns(
  ...
  std::transform(dones.begin(), dones.end(), std::back_inserter(not_dones), [](bool done) { return !done; });
...
std::vector<double> calc_q_value_logits(
    const std::vector<double>& rewards,
    const std::vector<bool>& dones,
    double next_v_pred,
    double gamma) {
