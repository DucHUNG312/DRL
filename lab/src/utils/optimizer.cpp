...
bool operator==(const RAdamOptions& lhs, const RAdamOptions& rhs) {
  return (lhs.lr() == rhs.lr()) && (std::get<0>(lhs.betas()) == std::get<0>(rhs.betas())) &&
      ...
      (lhs.weight_decay() == rhs.weight_decay()) && (torch::all(lhs.buffer() == rhs.buffer()).item<bool>());
}
...
bool operator==(const RAdamParamState& lhs, const RAdamParamState& rhs) {
  return (lhs.step() == rhs.step()) && torch::equal(lhs.exp_avg(), rhs.exp_avg()) &&
...
torch::Tensor RAdam::step(Optimizer::LossClosure closure /*= nullptr*/) {
  ...
  for (auto& group : param_groups_) {
    for (auto& p : group.params()) {
      ...
      TORCH_CHECK(
          !grad.is_sparse(), "RAdam does not support sparse gradients" /*, please consider SparseAdam instead*/);
