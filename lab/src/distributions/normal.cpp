...
Normal::Normal(const torch::Tensor& loc, const torch::Tensor& scale) : ExponentialFamily(loc.sizes()) {
...
torch::Tensor Normal::log_prob(const torch::Tensor& value) {
  return (-((value - loc_).pow(2)) / (2 * variance_) - scale_.log() - std::log(std::sqrt(2 * math::Pi)));
}
...
torch::Tensor Normal::log_normalizer(torch::TensorList params) {
  LAB_CHECK(params.size() == 2, "Expect params size is 2, got " << params.size());
