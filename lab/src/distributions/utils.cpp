...
torch::Tensor clamp_probs(const torch::Tensor& probs, const std::optional<torch::Device>& device /*= std::nullopt*/)
{
    ...
    return device.has_value() ? torch::clamp(probs, eps, 1. - eps).to(device.value()) : torch::clamp(probs, eps, 1. - eps);
} 

torch::Tensor probs_to_logits(const torch::Tensor& probs, bool is_binary /*= false*/, const std::optional<torch::Device>& device /*= std::nullopt*/)
{
...
torch::Tensor logits_to_probs(const torch::Tensor& logits, bool is_binary /*= false*/, const std::optional<torch::Device>& device /*= std::nullopt*/)
{
    if(is_binary) 
        return device.has_value() ? torch::sigmoid(logits).to(device.value()) : torch::sigmoid(logits);
    return device.has_value() ? torch::nn::functional::softmax(logits, {-1}).to(device.value()) : torch::nn::functional::softmax(logits, {-1});
}
