...
Body::Body(const std::shared_ptr<envs::Env>& env, const utils::BodySpec& spec) : env_(env), spec_(std::move(spec)) {
...
void Body::init_algorithm_specs(const utils::AlgorithmSpec& spec) {
  ...
  algorithm_->optimizer(
      utils::create_optim(algorithm_->net()->spec().optim_spec.name, algorithm_->net()->parameters()));
  algorithm_->lrscheduler(
      utils::create_lr_schedular(algorithm_->optimizer(), algorithm_->net()->spec().lr_scheduler_spec));
...
torch::serialize::OutputArchive& operator<<(
  ...
  LAB_CHECK(body != nullptr, "Body pointer is nullptr");
  body->save(archive);
  return archive;
}
