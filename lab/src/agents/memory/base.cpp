...
Memory::Memory(const utils::MemorySpec& spec) : spec_(std::move(spec)) {
  reset();
}
...
void Memory::pretty_print(std::ostream& stream, const std::string& indentation) const {
  ...
  // pretty_print_list(stream, "next_state", next_indentation, [](const
  // torch::IValue& value) { return value.toTensor();
  // });
...
torch::serialize::OutputArchive& operator<<(
  ...
  LAB_CHECK(memory != nullptr, "Memory pointer is nullptr");
  memory->save(archive);
  return archive;
}
