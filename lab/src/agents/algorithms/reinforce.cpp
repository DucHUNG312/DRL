...
torch::Tensor Reinforce::calc_policy_loss(
    const Algorithm::ExperienceDict& experiences,
    const torch::Tensor& states,
    const torch::Tensor& advs) {
  std::shared_ptr<distributions::Distribution> action_pd =
      utils::init_action_pd(spec_.action_pdtype, states.clone().to(torch::kCPU));
