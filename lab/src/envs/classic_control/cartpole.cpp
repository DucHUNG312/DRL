...
CartPole::CartPole() : CartPole(utils::SpecLoader::load_default_env_spec(name)) {}

CartPole::CartPole(const utils::EnvSpec& env_spec) : Env(env_spec) {
  LAB_CHECK(gravity >= 0, "Expect gravity greater or equal to 0, got " << gravity);
  LAB_CHECK(masscart >= 0, "Expect masscart greater or equal to 0, got " << masscart);
  LAB_CHECK(masspole >= 0, "Expect masspole greater or equal to 0, got " << masspole);
  LAB_CHECK(total_mass >= 0, "Expect total_mass greater or equal to 0, got " << total_mass);
  LAB_CHECK(length >= 0, "Expect length greater or equal to 0, got " << length);
  LAB_CHECK(polemass_length >= 0, "Expect polemass_length greater or equal to 0, got " << polemass_length);
  LAB_CHECK(force_mag >= 0, "Expect force_mag greater or equal to 0, got " << force_mag);
  LAB_CHECK(tau >= 0, "Expect tau greater or equal to 0, got " << tau);
  LAB_CHECK(
      theta_threshold_radians >= -math::Pi,
      "Expect theta_threshold_radians greater or equal to -Pi, got " << theta_threshold_radians);
  LAB_CHECK(
      theta_threshold_radians <= math::Pi,
      "Expect theta_threshold_radians smaller or equal to Pi, got " << theta_threshold_radians);
  LAB_CHECK(
      reset_low < reset_high, "Expect reset_low smaller than reset_high, got " << reset_low << " and " << reset_high);

  torch::Tensor high =
      torch::tensor({x_threshold * 2, math::Max, theta_threshold_radians * 2, math::Max}, torch::kDouble);
  ...
  //     registry_.add_component<renderer::RigidBody2DComponent>(entity_id,
  //     total_mass);
  // }
}
...
void CartPole::reset(uint64_t seed /*= 0*/) {
  ...
  // {
  //     registry_.add_or_replace_component<StepResultComponent>(entity_id,
  //     utils::get_data_from_tensor(result_.state), 0, false, false);
  //     registry_.add_or_replace_component<CartPoleActionComponent>(entity_id,
  //     0);
  // }
}
...
void CartPole::step(const torch::Tensor& action) {
  ...
  LAB_CHECK(action_spaces_->template ptr<spaces::Discrete>()->contains(action), "Input is not in action space");
  ...
  double thetaacc =
      (gravity * sintheta - costheta * temp) / (length * (4.0 / 3.0 - masspole * std::pow(costheta, 2) / total_mass));
  ...
  bool terminated = (x < -x_threshold) || (x > x_threshold) || (theta < -theta_threshold_radians) ||
  ...
  if (!terminated) {
    ...
    if (steps_beyond_terminated == 0)
      LAB_LOG_WARN(
          ...
          "already returned terminated = true. You should always call "
          "'reset()' once you receive 'terminated = true' -- any "
  ...
  //     action.item<int64_t>());
  //     registry_.add_or_replace_component<StepResultComponent>(entity_id,
  //     utils::get_data_from_tensor(result_.state), result_.reward,
  //     result_.terminated, result_.truncated);
  // }
}
