...
DiscreteOptions::DiscreteOptions(int64_t n, int64_t start) : n_(n), start_(start) {}

DiscreteImpl::DiscreteImpl(const DiscreteOptions& options_) : options(options_) {
  reset();
}
...
void DiscreteImpl::pretty_print(std::ostream& stream) const {
  stream << std::boolalpha << "lab::spaces::Discrete(n=" << options.n() << ", start=" << options.start() << ")";
}
...
torch::Tensor DiscreteImpl::sample(/*const torch::Tensor& mask*/) {
  ...
  //     torch::Tensor random_index = start +
  //     valid_indices[rand_.sample_int_uniform(0, valid_indices.size(0))];
...
bool DiscreteImpl::contains(const torch::Tensor& x) const {
  LAB_CHECK(x.numel() == 1, "Expect input is a scalar tensor");
  LAB_CHECK(x.dtype() == torch::kInt64, "Only support input with type torch::Int64");
