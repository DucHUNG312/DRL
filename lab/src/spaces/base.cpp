...
torch::Tensor& Space::register_parameter(std::string name, torch::Tensor tensor, bool requires_grad /*= false*/) {
  LAB_CHECK(!name.empty(), "Coundn't register space with empty name");
  LAB_CHECK(name.find('.') == std::string::npos, "Coundn't register space with character '.' in name");
  if (!tensor.defined())
    if (requires_grad) {
      LAB_LOG_WARN(
          "An undefined tensor cannot require grad. Ignoring the "
          "`requires_grad=true` function parameter.");
    } else {
...
torch::Tensor& Space::get_parameter(std::string name) {
  LAB_CHECK(!name.empty(), "Invalid name space");
  LAB_CHECK(name.find('.') == std::string::npos, "Invalid name space");
  LAB_CHECK(!parameters_.is_empty(), "Parameter is empty");

  if (parameters_.contains(name))
    return parameters_[name];
  LAB_THROW("No key with name " << name << " exists in parameters!");
}
...
torch::serialize::OutputArchive& operator<<(
  ...
  LAB_CHECK(space != nullptr, "Space pointer is nullptr");
  space->save(archive);
  return archive;
}
