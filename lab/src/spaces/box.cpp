...
BoxImpl::BoxImpl(const BoxOptions& options_) : options(options_) {
  reset();
}
...
void BoxImpl::reset() {
  ...
  LAB_CHECK(
      low.sizes() == high.sizes(),
      "Expect low and high have equal size, got " << low.sizes() << " and " << high.sizes());
...
Box make_box_space(torch::IntArrayRef shape /*= torch::IntArrayRef(1)*/) {
  return static_cast<Box>(
      std::make_shared<BoxImpl>(torch::full(shape, 0, torch::kDouble), torch::full(shape, 1, torch::kDouble)));
}
